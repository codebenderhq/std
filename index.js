const uri =
  "https://github.com/codebenderhq/backpack/releases/latest/download/grape.js";
...
const tailwindRun = async (isBuild) => {
  ...
  const styleCMD =
    `./tailwindcss -i ./src/input.css -o ./src/public/output.css  ${
      isBuild ? "--minify" : "--watch"
    }`.split(" ");
...
const new_project = async (name) => {
  const frame_installer =
    "curl -sLO https://github.com/codebenderhq/backpack-frame/archive/refs/heads/main.zip";
...
if (import.meta.main) {
  ...
  if (args.length > 0) {
    switch (args[0]) {
      ...
      case "help":
        console.log(
          "Setup your enviroment here --> https://codebenderhq.notion.site/4806ddc648e644d38e2223793a6a815e",
        );
        break;
      case "upgrade":
        upgrade();
